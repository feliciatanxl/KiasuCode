...
export function PrivateChat({
  ...
  useEffect(() => {
    ...
    const initChat = async () => {
      ...
      try {
        // 1. Ensure current user's RSA key pair is active
        ...
        for (const item of historyResponse.data.messages) {
          const plaintext = await decryptMessage(item.encryptedContent, user.id)
          decryptedList.push({
            id: item.id,
            senderId: item.senderId,
            senderName: item.senderId === user.id ? 'You' : friend.name,
  ...
  useEffect(() => {
    ...
    const handleIncomingPrivateMessage = async (msg: {
      ...
      const plaintext = await decryptMessage(msg.encryptedContent, user.id)
      ...
      setMessages((prev) => {
        ...
        return [
          ...prev,
          {
            ...
            senderName: msg.senderId === user.id ? 'You' : msg.senderName,
  ...
  const handleSendMessage = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // 1. Encrypt plaintext using friend's RSA public key (hybrid AES-GCM)
      const encryptedContent = await encryptMessage(text, friendPublicKey)

      // 2. Emit encrypted payload over WebSocket to server
    ...
    >
      ...
      >
        ...
        <div className="bg-amber-50/70 dark:bg-amber-950/30 px-4 py-2 border-b border-amber-100 dark:border-amber-900/40 text-[11px] text-amber-800 dark:text-amber-300 flex items-center gap-2">
          <span>🛡️</span>
          <span>
            <strong>End-to-End Encrypted</strong>: Plaintext never touches the server. Only you and {friend.name} hold the private decryption keys.
          </span>
        </div>
